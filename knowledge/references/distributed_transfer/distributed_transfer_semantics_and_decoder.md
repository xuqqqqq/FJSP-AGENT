...
合法性检查按每个完整资源二元组的开始时间排序后检查重叠，不按job/op输出顺序推断机器顺序。
转运等待不占用加工机器以外的额外资源；当前实例没有车辆容量约束，不自行添加。
工厂负载仅累计所选加工时长，不包含转运等待或机器空闲。能耗必须从实际选中的duration和
unit_energy元组重算，不能因为machine相同就复用旧能耗，也不能把延迟与单位能耗相乘。
