...
实际 DAG/传播必须把 job ready、machine predecessor、setup interval 和 processing interval 按契约连接。机器顺序、插入、交换、换机、交叉或重启后，前驱改变的 setup 必须重算；不能沿用标准 FJSP 增量更新或旧的 start/end。
