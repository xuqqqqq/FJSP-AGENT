...
把这里当作压缩后的 prompt 证据，不要当作实现已完成的证明。

## AWLS / HGTSA 家族

- AWLS 风格的 FJSP 搜索通常使用析取图排程、关键路径/关键块邻域、tabu 记忆和自适应工序权重。
- N7/N8 同机 move 会围绕关键块重定位工序。
- NK 或 RK/LK 换机插入会利用 head/tail 信息收缩目标位置，而不是扫描每个插入点。
- `zi` 通过工序权重和 cooldown 扰动近似 move 评分，但只有在底层时间模型正确后才有意义。

## FJSP-SDST 文献模式

- sequence-dependent setup time 会占用同一台机器上相邻两道工序之间的机器容量。
- 只看 processing time 似乎有利的 move，在计入 setup 插入/移除成本后可能变差。
