...
type: reference
title: 最小时间间隔 FJSP 语义、时间图与解码合同
tags: [fjsp, minimum_time_lag, time_lag, lag_aware_decoder, difference_constraints]
status: active
---

# 最小时间间隔 FJSP 语义、时间图与解码合同

## 适用范围

本卡只覆盖当前 `fjsp_min_time_lag` 合同：标准 FJSP 主体后追加固定约束列表，每条约束只连接同一工件的相邻工序，且与机器选择无关。
...
`L >= 0`。未列出的相邻工序只保留标准前驱约束；`L=0` 与标准前驱约束等价。

以下内容不在本卡范围内：maximum lag、no-wait 等式、跨工件 lag、依赖机器选择的运输 lag、SDST、lag buffer 容量或随机 lag。

## 不占机语义

lag 发生在前驱工序完工之后，约束工件后继工序的释放时间，但不占用前驱机器。

- 前驱机器在 `end[j,k]` 立即释放。
- 工件的下一道工序在 `end[j,k] + L` 才 ready。
...
节点距离表示工序开始时刻。对任一前驱弧 `(u,v,w)`，都要求 `start(v) >= start(u) + w`。
...
6. 在 SCC 凝聚后的 DAG 上按拓扑序计算最长前驱距离；同一零权 SCC 内节点使用相同的最早开始时刻。
7. 输出完整排程，并再次检查每道工序一次、机器不重叠和 lag 不变量。
...
机器空隙只解决 machine availability，不能替代 job release：候选开始时刻至少为 `pred_end + L`。把操作插入已有机器序列中间后，受影响的不只是该操作；其机器后继和工件后继可能继续右移。因此：
...
忽略机器容量、对每道未排工序取最短加工时间，可得到工件链下界：
...
1. **机器释放**：工序 A 完成后进入长 lag，同机另一工件必须能立即加工。
2. **边界等号**：后继在 `end(A)+L` 开始应合法，早 1 个时间单位应非法。
3. **局部移动传播**：换机或交换使前驱推迟时，工件后继链必须同步推迟。
...
前两项覆盖更一般的时间窗/广义 FJSP；最后一项与当前 machine-free finish-start min-lag 最接近。本文只保留三者中对当前固定相邻 minimum lag 可证明成立的部分。
