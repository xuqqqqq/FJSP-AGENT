...
时长。smoke 应从提取结果逐工序验证这个不变量，并重算目标；不能因为 CP-SAT 对错误模型返回
...
只从 `FEASIBLE/OPTIMAL` 导出候选；`UNKNOWN/INFEASIBLE/MODEL_INVALID`、超时或异常均返回原 incumbent，且 `UNKNOWN` 不调用 `solver.Value(...)`。短 probe 只测试模型响应，主要求解应获得连续预算，避免大量重建模型。
