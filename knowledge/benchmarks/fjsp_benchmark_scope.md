...
标准 FJSP 的回归范围使用 `FJSP-Instance-main/instance` 下的四个本地家族：
...
这个本地包的参考界记录在 `knowledge/benchmarks/standard_fjsp_bounds_LB_UB.csv` 中。界值来源于 FJSPLib 元数据，以及 JSPLib 对 Hurink `sdata` 实例的经典元数据；本地包把这些实例说明为与 JSP 案例等价。当前固定 CSV 中有 8 个 HU `sdata-car*` 实例没有显式 LB/UB 行，对它们应报告为缺失界值，而不是静默映射到其他 Hurink 变体。
...
- 只要存在 bounds，就始终报告 `family_label`、`instance`、validity、makespan、LB、UB/BKS、到 LB 的 gap 和到 UB 的 gap。
