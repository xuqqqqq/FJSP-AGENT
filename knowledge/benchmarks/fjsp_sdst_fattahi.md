...
随包附带的 `README.txt` 说明，这些文件是来自以下来源的公开 FJSP 实例，并带有 sequence-dependent setup times：
...
`Fattahi_setup_01.fjs` 以标准 FJSP 风格的头部和作业工序候选机器信息开始：
...
随后文件包含一个独立的 setup-time 区块。对这个微型实例来说，检查到的区块有 8 行：
...
- 能读取额外 setup-time 矩阵的 parser；
- 基于每台机器前一道 operation 插入 setup time 的 evaluator；
- 明确说明 setup interval 是在 output 中显式给出，还是由 evaluator 重新计算的解合同；
- 为 setup-aware neighborhood move 和 insertion scoring 单独准备方法包组件。
