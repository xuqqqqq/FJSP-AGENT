...
For an agent-generated solver, a compact decoder and one bounded neighborhood
are a low-risk starting recommendation, not a mandatory stopping point. The
Agent may instead design a coherent full AWLS adaptation when the selected
direction, incumbent, evidence, and budget justify that scope. In either case,
do not mechanically port local AWLS source: map the representation, decoder,
move lifecycle, search control, and SDST semantics to the active contract, and
keep operation-coverage checks executable.
