...
平台后端应保持问题族层面的通用性。变体特定的算法、论文笔记和修复规则，应放在领域包、知识卡、Skill、方法包以及有边界的 Worker Assignment 中。某次运行里失败的尝试应写入 `experiment_memory/`，而不是沉淀为稳定的方法指导。
...
4. 按标签和单个方法包选择知识；不要用无关论文淹没 Worker。
...
- Method、Skill 和方法包指导应描述可复用机制、不变量和失败模式，而不是某个实例的目标 makespan、拷贝来的 schedule 或依赖 seed 的答案。
- 数值 makespan、LB/UB/BKS 以及逐实例 gap 应放在 benchmark、capability 或 experiment 报告中。它们是 Main 侧用于门槛和比较的诊断信息，不是 Worker solver 输入。
- 当把实验结论提升为知识卡时，要把具体结果归一化成方法层经验，例如“operation-level 的 setup-aware dispatch 有帮助”或“混合表示的 local search 失败了”。
