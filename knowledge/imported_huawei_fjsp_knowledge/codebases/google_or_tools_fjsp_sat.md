...
1. 可选机器建模：每个 operation 有多个 optional interval。
2. 机器容量建模：同一机器的 interval 加 `NoOverlap`。
3. 工件前后关系：同一 job 内增加 precedence。
4. 目标：最小化所有工序最大完工时间。
...
3. 对工业华为算例的组批、转运和复杂 setup 需要额外建模。

## 后续动作

1. 先写一个 Barnes 小实例 CP-SAT baseline，时间限制 10-60 秒。
2. 用 CP-SAT 解验证我们的解码器和校验器是否口径一致。
3. 对较小 Barnes 实例记录 lower bound / best feasible，作为自演进的强基线。
