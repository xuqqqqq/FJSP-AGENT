# 现有 Skill（技能）与知识库分类清单

盘点日期：2026-07-25。清单覆盖项目级 `.codex/skills`、OpenCode 内部 `.opencode/skills`、`knowledge/` 和标准 FJSP 领域包（Domain Pack）。
...
| `fjsp-awls-sdst-adapter` | 完整方法适配规划 | AWLS 到 SDST 的方法包选择和适配审查 |
| `fjsp-variant-domain-pack` | 领域扩展 | 新变体的领域包、RAG、Skill 和 evaluator 边界设计 |
...
| `fjsp-experiment-design-worker` | 实验执行 | 最小插桩、激活检查、试跑、消融和证据回报 |
...
### 1.4 OpenCode 内部 Skill（技能）

| Skill（技能） | 用途 |
| --- | --- |
| `algoforge-assignment` | 读取受控 WorkerAssignment 并执行限定代码任务 |
| `experiment-design` | 通用试跑、消融、证据与交接流程 |
...
## 3. 领域包（Domain Pack）注册关系

`domain_packs/standard_fjsp/domain_pack.json` 当前声明：

- 4 个可组合方法族：构造搜索、耦合局部搜索、精确混合、种群/模因。
- 8 个可自动授权的 Worker Skill：2 个默认始终包含的基础 Skill、4 个方法族 Skill、1 个按高柔性标签激活的组合 Skill、1 个 SDST 横切 Skill。
- 2 个完整方法包。
...
以下内容与执行合同紧耦合，继续保留为 Skill 本地参考资料：

- 独立 solver IO/安全合同。
- Worker 实验模板与证据交接格式。
- 基础状态/完整解码合同。

当前领域包中只剩独立 solver 合同这类紧耦合 Skill 参考资料；通用算法检索路径均落在知识库。
