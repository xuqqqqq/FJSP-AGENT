...
该方向强调在 FJSP 中结合全局搜索和局部邻域搜索，并使用禁忌搜索强化局部改进。对当前项目最有价值的是“邻域设计”思想：不能只根据单步派工评分做构造式调度，还要在完整排程上识别关键结构并重排。

## 适用问题

适合标准 FJSP 的最大完工时间优化。若扩展到顺序相关 setup、维修窗口或组批机台，需要让邻域移动经过可行性过滤或重解码。
...
1. Barnes 差距最大的 mt10x/mt10xxx 接近 18%，应优先查看关键路径和机器块，而不是继续调派工权重。
...
3. 用 Barnes 相对公开已知最优结果的差距作为是否有效的判据。
