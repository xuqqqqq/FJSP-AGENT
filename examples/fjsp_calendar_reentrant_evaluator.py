...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate release/calendar/reentrant FJSP schedules."
    )
    ...
    try:
        ...
        if instance.variant != "fjsp_calendar_reentrant":
            raise ValueError(
                "calendar-reentrant evaluator requires an fjsp_calendar_reentrant instance"
            )
        if not (
            instance.has_release_times
            and instance.has_machine_availability
            and instance.has_reentrant_routes
        ):
            raise ValueError("calendar-reentrant instance is missing an active feature")
