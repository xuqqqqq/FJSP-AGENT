...
class AwlsSlotModeTests(unittest.TestCase):
    ...
    def test_tabu_memory_slot_warns_on_importing_nonexistent_operation_key(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_tabu_memory")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "setup_pressure_tenure",
                        "type": "tabu_memory_rule",
                        "novelty": "Uses setup pressure without changing the tabu sequence.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "slot_id": "awls_sdst_tabu_memory",
                        "content": (
                            "    from harness_agent.standard_fjsp import setup_time_between, operation_key\n"
                            "    machine_id = schedule.on_machine[move.which]\n"
                            "    sequence = [move.which]\n"
                            "    prev_op = operation_key(schedule, sequence[0])\n"
                            "    tenure = tenure_min\n"
                            "    tabu.add(machine_id, sequence, iteration + tenure)\n"
                        ),
                    }
    ...
    def test_tabu_memory_slot_warns_on_nonexistent_move_duration(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_tabu_memory")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "duration_scaled_tenure",
                        "type": "tabu_memory_rule",
                        "novelty": "Scales tenure by moved operation duration.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            "    machine_id = schedule.on_machine[move.which]\n"
                            "    sequence = [move.which]\n"
                            "    tenure = tenure_min + int(move.duration)\n"
                            "    tabu.add(machine_id, sequence, iteration + tenure)\n"
                        ),
                    }
                ],
    ...
    def test_tabu_memory_slot_warns_on_nonexistent_machine_head_or_jobs(self) -> None:
        ...
        first_machine_normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "bottleneck_aware_critical_tenure",
                        "type": "tabu_memory_rule",
                        "novelty": "Scales tenure by a bottleneck machine load estimate.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            "    machine_id = schedule.on_machine[move.which]\n"
                            "    machine_count = len(schedule.first_machine)\n"
                            "    machine_load = 0\n"
                            "    node = schedule.first_machine[machine_id]\n"
                            "    while node != -1:\n"
                            "        machine_load += schedule.index.duration(node, machine_id)\n"
                            "        node = schedule.machine_successor[node]\n"
                            "    tenure = tenure_min + int(machine_count > 0 and machine_load > 0)\n"
                            "    tabu.add(machine_id, [move.which], iteration + tenure)\n"
                        ),
                    }
                ],
        ...
        self.assertIn(
            "tabu_memory_uses_nonexistent_machine_head_or_jobs",
            first_machine_normalized["proposal_audit"]["warnings"],
        )
        self.assertTrue(generic_slot_needs_repair(first_machine_normalized))
        self.assertIn("first_machine", guidance)
    ...
    def test_tabu_memory_slot_warns_on_setup_without_import(self) -> None:
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "setup_delta_tenure_missing_import",
                        "type": "tabu_memory_rule",
                        "novelty": "Different from failed setup ratio tenure by using documented operation keys.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "slot_id": "awls_sdst_tabu_memory",
                        "content": (
                            "    machine_id, sequence = candidate_tabu_sequence_parts(schedule, move.method, move.which, move.where)\n"
                            "    prev_op = operation_key(schedule, sequence[0])\n"
                            "    cur_op = operation_key(schedule, sequence[-1])\n"
                            "    setup = setup_time_between(schedule.index.instance, machine_id, prev_op, cur_op, schedule.index)\n"
                            "    expires_at = iteration + schedule.rng.randint(tenure_min, tenure_max) + int(setup > 0)\n"
                            "    tabu.add(machine_id, sequence, expires_at)\n"
                        ),
                    }
                ],
            },
        ...
        self.assertIn("tabu_memory_uses_setup_without_import", normalized["proposal_audit"]["warnings"])
        self.assertTrue(generic_slot_needs_repair(normalized))

    def test_tabu_memory_slot_warns_on_nonexistent_is_critical_api(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_tabu_memory")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "critical_setup_tenure",
                        "type": "tabu_memory_rule",
                        "novelty": "Different from failed critical-only tenure by combining setup pressure.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            "    machine_id, sequence = candidate_tabu_sequence_parts(schedule, move.method, move.which, move.where)\n"
                            "    tenure = tenure_max if schedule.is_critical(move.which) else tenure_min\n"
                            "    tabu.add(machine_id, sequence, iteration + tenure)\n"
                        ),
                    }
                ],
    ...
    def test_tabu_memory_slot_warns_on_target_machine_change_tabu_retry(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_tabu_memory")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "target_machine_tabu_for_change_moves",
                        "type": "tabu_memory_rule",
                        "novelty": "Different from sequence-length tenure by placing change-move tabu on the target machine.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            ...
                            "        machine_id, sequence = candidate_tabu_sequence_parts(schedule, move.method, move.which, move.where)\n"
                            "    else:\n"
                            "        target_machine = schedule.on_machine[move.where]\n"
                            "        sequence = [move.where, move.which]\n"
                            "        machine_id = target_machine\n"
                            "    if move.method in (CHANGE_MACHINE_FRONT, CHANGE_MACHINE_BACK):\n"
    ...
    def test_tabu_memory_repair_guidance_names_single_tabu_add_contract(self) -> None:
        ...
        self.assertIn("target-machine", guidance)
    ...
    def test_portfolio_slot_warns_on_best_lane_rerun_retry(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_portfolio_search_control")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "probe_then_deepen_best_lane",
                        "type": "search_control",
                        "novelty": "Uses a probe phase and reruns the best lane.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            ...
                            "        candidate = solve_awls_single(index, seed=lane.seed, restarts=lane.restarts, cycles_per_restart=cycles_per_restart, iterations=iterations, time_limit_sec=lane_budget, init_mode=lane.init_mode, beta=beta, gamma=gamma, theta=theta, exact_select_top_k=exact_select_top_k, same_machine_eval=same_machine_eval, critical_block_exhaustive_pct=critical_block_exhaustive_pct, zi_policy=zi_policy, zi_formula=zi_formula, initial_state=initial_state, time_check_interval=time_check_interval, cycle_trace=cycle_trace)\n"
                            "        if best is None or candidate.makespan < best.makespan:\n"
                            "            best = candidate.clone()\n"
                            "            best_lane = lane\n"
                            "    remaining_time = max(0.0, time_limit_sec - sum(lane_budgets))\n"
                            "    if best_lane is not None and remaining_time > 0:\n"
                            "        candidate = solve_awls_single(index, seed=best_lane.seed, restarts=best_lane.restarts * 2, cycles_per_restart=cycles_per_restart, iterations=iterations, time_limit_sec=remaining_time, init_mode=best_lane.init_mode, beta=beta, gamma=gamma, theta=theta, exact_select_top_k=exact_select_top_k, same_machine_eval=same_machine_eval, critical_block_exhaustive_pct=critical_block_exhaustive_pct, zi_policy=zi_policy, zi_formula=zi_formula, initial_state=initial_state, time_check_interval=time_check_interval, cycle_trace=cycle_trace)\n"
                        ),
                    }
                ],
            },
        ...
        self.assertIn("portfolio_retries_best_lane_rerun", normalized["proposal_audit"]["warnings"])
        self.assertTrue(generic_slot_needs_repair(normalized))

    def test_portfolio_slot_warns_on_subrun_seed_splitting_retry(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_portfolio_search_control")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "three_subrun_lane_split",
                        "type": "search_control",
                        "novelty": "Splits each lane into deterministic subruns.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            ...
                            "            seed_offset = sub_idx * 123457\n"
                            "            candidate = solve_awls_single(index, seed=lane.seed + seed_offset, restarts=lane.restarts, cycles_per_restart=cycles_per_restart, iterations=iterations, time_limit_sec=lane_budget / 3.0, init_mode=lane.init_mode, beta=beta, gamma=gamma, theta=theta, exact_select_top_k=exact_select_top_k, same_machine_eval=same_machine_eval, critical_block_exhaustive_pct=critical_block_exhaustive_pct, zi_policy=zi_policy, zi_formula=zi_formula, initial_state=initial_state, time_check_interval=time_check_interval, cycle_trace=cycle_trace)\n"
                        ),
                    }
                ],
            },
        ...
        self.assertIn("portfolio_retries_subrun_seed_splitting", normalized["proposal_audit"]["warnings"])
        self.assertTrue(generic_slot_needs_repair(normalized))

    def test_portfolio_slot_warns_on_setup_ratio_best_lane_exploitation_retry(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_portfolio_search_control")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "setup_ratio_best_lane_exploitation",
                        "type": "search_control",
                        "novelty": "Uses setup ratio to intensify the best lane.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            "    lane_budgets = allocate_lane_budgets(portfolio_lanes, time_limit_sec)\n"
                            "    setup_ratio = 0.42\n"
                            "    best_lane = portfolio_lanes[0]\n"
                            "    if setup_ratio > 0.25:\n"
                            "        candidate = solve_awls_single(index, seed=best_lane.seed, restarts=best_lane.restarts * 2, cycles_per_restart=cycles_per_restart, iterations=iterations, time_limit_sec=time_limit_sec, init_mode=best_lane.init_mode, beta=beta, gamma=gamma * 2, theta=theta, exact_select_top_k=exact_select_top_k, same_machine_eval=same_machine_eval, critical_block_exhaustive_pct=critical_block_exhaustive_pct * 2, zi_policy=zi_policy, zi_formula=zi_formula, initial_state=initial_state, time_check_interval=time_check_interval, cycle_trace=cycle_trace)\n"
                        ),
                    }
                ],
            },
    ...
    def test_portfolio_slot_warns_on_dispatch_warmstart_noop_retry(self) -> None:
        worker = DeepSeekSlotWorker()
        context = _generic_slot_context(slot_id="awls_sdst_portfolio_search_control")
        slot = context["slot_manifest"]["slots"][0]
        ...
        normalized = worker._normalize_generic_slot_proposal(  # noqa: SLF001 - regression-tests worker normalization.
            {
                "rule_operator_hypotheses": [
                    {
                        "name": "sdst_aware_dispatch_warmstart",
                        "type": "search_control",
                        "novelty": "Uses dispatch warm starts instead of changing lane budgets.",
                        "target_files": ["examples/standard_fjsp_awls_solver.py"],
                    }
                ],
                "changes": [
                    {
                        ...
                        "content": (
                            ...
                            "        for rule in ['mwkr', 'lpt', 'spt', 'edd']:\n"
                            "            schedule = _dispatch_schedule(index, rule)\n"
                            "            warm_start = schedule\n"
                            "        candidate = solve_awls_single(index, seed=effective_lane_seed, restarts=lane.restarts, cycles_per_restart=cycles_per_restart, iterations=iterations, time_limit_sec=lane_budget, init_mode=lane.init_mode, beta=beta, gamma=gamma, theta=theta, exact_select_top_k=exact_select_top_k, same_machine_eval=same_machine_eval, critical_block_exhaustive_pct=critical_block_exhaustive_pct, zi_policy=zi_policy, zi_formula=zi_formula, initial_state=warm_start, time_check_interval=time_check_interval, cycle_trace=cycle_trace)\n"
                        ),
                    }
                ],
            },
        ...
        self.assertTrue(generic_slot_needs_repair(normalized))
        guidance = generic_slot_repair_guidance(slot)
        self.assertIn("dispatch warm-starts", guidance)
        self.assertIn("1302.50", guidance)
