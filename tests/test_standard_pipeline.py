...
class StandardPipelineTests(unittest.TestCase):
    ...
    def test_standard_pipeline_runs_all_core_stages(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(1, memory["direction_graph_signal"]["direction_count"])
            self.assertGreaterEqual(memory["experience_memory_signal"]["candidate_lesson_count"], 1)
            self.assertIn("record_count", memory["skill_usage_signal"])
