...
DEFAULT_SLOT_REPAIR_GUIDANCE = (
    "- Keep the replacement inside the selected slot contract and make the novelty materially different from failure memory."
)
...
def generic_slot_repair_guidance(slot: dict[str, Any]) -> str:
    slot_id = str(slot.get("slot_id") or "")
    problem_family = str(slot.get("problem_family") or "standard_fjsp")
    payload = load_edit_strategy_json_asset(
        problem_family=problem_family,
        strategy_name="slot_based_edit",
        asset_key="slot_repair_guidance",
    )
    guidance_by_slot = payload.get("slot_guidance") if isinstance(payload, dict) else None
    if isinstance(guidance_by_slot, dict):
        guidance = guidance_by_slot.get(slot_id)
        if isinstance(guidance, str) and guidance.strip():
            return guidance
    default_guidance = payload.get("default_guidance") if isinstance(payload, dict) else None
    if isinstance(default_guidance, str) and default_guidance.strip():
        return default_guidance
    return DEFAULT_SLOT_REPAIR_GUIDANCE
